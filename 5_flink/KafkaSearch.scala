...
class KafkaSearch (target:String) {

    val inputTopics: ArrayList[String] = new ArrayList[String]() {
      {
      add("mn_buy_ticket_1") //车票购买记录主题
      add("mn_hotel_stay_1") //酒店入住信息主题
      add("mn_monitoring_1") //监控系统数据主题
    ...
    inputKafkaStream.filter(x => x.get("value").get("username").asText("").equals(target)).map(x => {
